...
fn configure_ecdsafail_submission_route() {
    ...
    // RACE-1217: fold-carry window 19 -> 17. This drops the fused double_y/halve_y
    // peak `floor + 42 + W_fold` by 2 (1218 -> 1216 in that phase) so it clears the
    // 1217 floor, and shaves avg-executed Toffoli (the dropped fold-carry-escape
    // bits are 0 on the reachable support, ~2^-W otherwise). Same truncation CLASS
    // the frontier already ships (it baked W=19); residual classical/phase failures
    // are the Fiat-Shamir island, dodged by re-hunting DIALOG_TAIL_NONCE.
    set_default_env("DIALOG_GCD_FOLD_CARRY_TRUNC_W", "17");
    // RACE-1217: the baked nonce was hunted for the OLD 1218 (SEG=191/F_BLOCKS=11/
    // W=19) stream. The 1217 levers below (F_BLOCKS=12, PROD65, SEG=190, W=17)
    // reseed SHAKE256(op_stream) -> this nonce is STALE (leaves ~31 classical+phase
    // stragglers = the displacement; ancilla-garbage stays 0 = value-exact). A
    // fresh DIALOG_TAIL_NONCE island hunt on THIS stream is required for a clean
    // 0/0/0 submission (GPU/fasteval step, not done here).
    ...
    // RACE-1217: apply ripple chunked into 12 (was 11) so the transient
    // load/carry register narrows enough to drop the apply_chunk_{add,sub}_ripple
    // peak 1218 -> 1215, clearing the 1217 floor. Costs ~+510 avg Toffoli (extra
    // boundary comparators); the round84/square/double_y cuts below more than
    // pay for it. Value-exact (pure circuit restructuring; ancilla-garbage 0).
    ...
    // RACE-1217: keep the widest monolithic square row at 191 (== zuiris) and host
    // its 3-qubit carry deficit on the row's own high bits via the Cuccaro
    // gate-suffix (maj/uma), dropping round84_inplace_solinas_square fwd/inv
    // 1218 -> 1217 at ~12 CCX on that one row -- cheaper (and lower-displacement)
    // than windowing row 63 via SEG=190. Value-exact: maj/uma is the ordinary
    // Cuccaro carry, the same product lands in tmp_ext, the inverse mirrors it
    // (ancilla-garbage 0). CEILING=1217 fires the host only on the over-1217 row.
    set_default_env("SQUARE_ROW_MAX_SEG", "191");
    set_default_env("ROUND84_SQ_CARRY_HOST", "1");
    set_default_env("ROUND84_SQ_CARRY_HOST_CEILING", "1217");
    // RACE-1217: round84 fold quotient*c product 66 -> 65 bits. product = quotient*c
    // < c^2 < 2^65, so product[65] is provably |0> (product[64] IS needed). Frees
    // one ancilla at the round84 fold/unfold peak -> 1218 -> 1217. Value-exact
    // (ancilla-garbage 0 over all 9024 shots; if bit 65 were ever set the unfold
    // would leave garbage). Orthogonal to the baked QPROD_VENT_PAD (which vents the
    // wide-shift carry ripple onto dirty lanes but does not shrink the register).
    set_default_env("ROUND84_PROD65", "1");
