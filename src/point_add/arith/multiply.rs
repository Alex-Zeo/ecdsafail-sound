...
thread_local! {
    // Per-row override for the selfhosted-square Cuccaro gate-suffix width. When
    // `Some(k)`, the next row add/sub ripples its top `k` bits in place via maj/uma
    // (the ordinary Cuccaro carry) and borrows only `width-k` carry lanes. Set by
    // the square's safe-reuse branch around the binding (widest) monolithic row
    // under ROUND84_SQ_CARRY_HOST=1, so the gate-suffix trade is paid ONLY where it
    // removes a peak qubit (not on every monolithic row). Both the `need`
    // computation and the Cuccaro internal call read this same override.
    static SQ_GATE_SUFFIX_OVERRIDE: std::cell::Cell<Option<usize>> = const { std::cell::Cell::new(None) };
}

fn sq_gate_suffix_override_get() -> Option<usize> {
    SQ_GATE_SUFFIX_OVERRIDE.with(|c| c.get())
}

fn sq_gate_suffix_override_set(v: Option<usize>) {
    SQ_GATE_SUFFIX_OVERRIDE.with(|c| c.set(v));
}

/// Lever: host the self-hosted square's per-window carry-lane DEFICIT on the
/// square's own high bits via the Cuccaro gate-suffix, instead of fresh global
/// ancillas. The symmetric square's widest monolithic row needs `width` clean
/// carry lanes; the clean tmp_ext tail (+ any clean_supplement) is exactly the
/// 3-bit diagonal/gap/pad overhead short for every monolithic row. The default
/// fills the deficit with fresh global ancillas, lifting the binding row to peak
/// `4n + width + deficit`. Under this knob that single binding row ripples its
/// top `deficit` bits in place (maj/uma), so `from_global` -> 0 and the row peaks
/// at `4n + width`. Value-exact: maj/uma is the ordinary Cuccaro carry; the same
/// product lands in tmp_ext and the inverse mirrors it. Alternative to windowing
/// the row via SQUARE_ROW_MAX_SEG (which here costs only ~+78 avg Toffoli, so this
/// lever is not baked — it is provided + tested for the redundancy record).
fn round84_sq_carry_host_enabled() -> bool {
    std::env::var("ROUND84_SQ_CARRY_HOST").ok().as_deref() == Some("1")
}

/// Peak ceiling the carry-host lever defends. A monolithic safe-reuse row is
/// hosted only if its natural peak `4n + width + deficit` would STRICTLY exceed
/// this (so the lever fires exactly on the over-ceiling binder and stays inert
/// elsewhere). 4n = tx+ty+tmp_ext is the live baseline during the lam^2 square.
fn round84_sq_carry_host_ceiling() -> usize {
    std::env::var("ROUND84_SQ_CARRY_HOST_CEILING")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(1216)
}

/// Per-row gate-suffix for the carry-host lever: the number of top row bits to
/// ripple in place. Fires ONLY on a monolithic safe-reuse row whose natural peak
/// `4n + width + deficit` strictly exceeds the ceiling, returning exactly the
/// clean-source deficit `raw_need - avail` there (zero elsewhere). `avail` is the
/// row's total clean carry budget (tmp_ext tail + clean_supplement).
fn round84_sq_carry_host_suffix(
    raw_need: usize,
    avail: usize,
    max_seg: usize,
    width: usize,
    ...
    if !round84_sq_carry_host_enabled() || max_seg == 0 || width < max_seg {
        return 0;
    }
    let deficit = raw_need.saturating_sub(avail);
    if 4 * n + width + deficit <= round84_sq_carry_host_ceiling() {
        return 0;
    }
    deficit
}
...
pub(crate) fn square_selfhost_gate_suffix_carries(n: usize) -> usize {
    if let Some(k) = sq_gate_suffix_override_get() {
        return k.min(n.saturating_sub(1));
    }
...
pub(crate) fn schoolbook_square_symmetric_lowq_selfhosted_with_clean_supplement(
    ...
    for i in 0..n {
        ...
        if i < gate_prefix_rows {
            ...
            // Carry-host lever: on the binding (widest) monolithic row, ripple the
            // global-ancilla deficit in place via maj/uma instead of allocating it,
            // so the row does not lift the peak. The override is read by BOTH the
            // `need` computation here and the Cuccaro internal gate-suffix.
            let total_avail = (tmp_ext.len() - hi) + clean_supplement.len();
            let host_suffix = round84_sq_carry_host_suffix(row.len(), total_avail, max_seg, width, n);
            if host_suffix > 0 {
                sq_gate_suffix_override_set(Some(host_suffix));
            }
            ...
            b.free_vec(&gpool);
            if host_suffix > 0 {
                sq_gate_suffix_override_set(None);
            }
        } else {
            let pad = b.alloc_qubit();
            let mut row_padded = row.clone();
...
pub(crate) fn schoolbook_square_symmetric_lowq_selfhosted_inverse_with_clean_supplement(
    ...
    for i in (0..n).rev() {
        ...
        if i < gate_prefix_rows {
            ...
            // Inverse mirror of the carry-host lever (see the forward branch).
            let total_avail = (tmp_ext.len() - hi) + clean_supplement.len();
            let host_suffix = round84_sq_carry_host_suffix(row.len(), total_avail, max_seg, width, n);
            if host_suffix > 0 {
                sq_gate_suffix_override_set(Some(host_suffix));
...
/// Lever: the fold `quotient*c` product is `< c^2 < 2^65`, so `product[65]` is
/// provably |0>. Allocate 65 bits instead of 66 to free one ancilla at the
/// round84 fold/unfold peak (value-exact: bit 65 is never set). `product[64]` IS
/// needed (c^2 > 2^64) so 64 would break; only the top carry bit is droppable.
/// Orthogonal to QPROD_VENT_PAD (which vents the wide-shift carry ripple onto
/// dirty lanes but does not shrink the product register).
fn round84_prod65_enabled() -> bool {
    std::env::var("ROUND84_PROD65").ok().as_deref() == Some("1")
}
...
fn round84_compute_quotient_c_product(b: &mut B, quotient: &[QubitId], dirty: &[QubitId]) -> Vec<QubitId> {
    ...
    // (65 bits with PROD65 — product < c^2 < 2^65, so product[65] is always |0>).
    let q = &quotient[..33];
    let product = b.alloc_qubits(if round84_prod65_enabled() { 65 } else { 66 });
    for i in 0..q.len() {
        b.cx(q[i], product[i]);
    }
